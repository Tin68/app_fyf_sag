...
class HTTPCookie(str):
    ...
    def get_from_router_data(self, instance: BaseState) -> "str | None":
        ...
        with contextlib.suppress(Exception):
            ws_cookie = SimpleCookie()
            ws_cookie.load(instance.router.headers.cookie)
            cookie_name = self.get_cookie_name()
            if (morsel := ws_cookie.get(cookie_name)) is not None:
                return morsel.value
        return None
...
async def sync_cookies(request: Request) -> Response:
    ...
    """
    if (origin := request.headers.get("origin")) is not None and urlsplit(
        origin
    ).netloc != request.base_url.netloc:
        console.warn(
            f"Cross-origin cookie sync attempt from {origin} to {request.base_url} is not allowed.",
            dedupe=True,
        )
        return Response("Cross-origin cookie sync is not allowed", status_code=403)
